...
Run from learning/llm-workshop-001; use a new output directory for every recording.
"""
...
def main():
    ...
    (folder/'record_experiments.py').write_text(Path(__file__).read_text())  # Preserve the recording source.
    session = Session()
    ...
    for corpus_name, corpus in corpora.items():
        for device in args.devices:
            ...
            for step in range(1, args.steps+1):
                ...
                model.train()  # Enable dropout for full-model training.
                optimizer.zero_grad(set_to_none=True)  # Clear old gradients.
                _, loss = model(x, y)  # Predict the shifted next-character targets.
                loss.backward()  # Calculate how each weight affects the error.
                torch.nn.utils.clip_grad_norm_(model.parameters(), 1., error_if_nonfinite=True)
                optimizer.step()  # Apply one update to the model parameters.
    ...
    for domain, prose in topic(6).DOMAIN_TEXTS.items():
        ...
        for step in range(1, args.adapter_steps+1):
            ...
            loss = model(x, y)[1]
            loss.backward()  # Only A and B have gradients; the base stays frozen.
            torch.nn.utils.clip_grad_norm_(parameters,1.,error_if_nonfinite=True)
            optimizer.step()  # Apply one adapter update.
